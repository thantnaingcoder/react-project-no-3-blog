...
export default function Posts() {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setShow(true)
      ...
      const {data, error ,isLoading} = useSWR("http://localhost:3000/blogs",fetcher)
  ...
  return (
    ...
    {show && <p>Search : {query}</p>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {isLoading && [0,1,2,3,4,5].map((i,index) => <Skeleton key={index}/>)}

        {   data?.map((post) => (
          <article onClick={() => nav(`/post/${post.id}`)} key={post.id} className="border rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
            <img 
              src={post.url} 

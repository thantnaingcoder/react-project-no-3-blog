...
export default function Layout() {
  ...
  return (
    <div className=" flex flex-col">
      <ScrollNav>
        <header
          className={`px-4 lg:px-6 h-14 flex  border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60  items-center border-b shadow-2xl  `}
        >
          <p
            onClick={() => nav("/home")}
          ...
          <nav className="ml-auto flex gap-5 items-center cursor-pointer sm:gap-6">
            ...
            <button
              onClick={() => nav("/posts")}
              className={`text-sm font-medium hover:underline underline-offset-4 ${
                location.pathname == "/posts" && "underline"
              }`}
            >
              Blogs
            </button>
            <button
              ...
              className={`text-sm font-medium hover:underline underline-offset-4 ${
                location.pathname == "/create-blog" && "underline"
              }`}
            >
              Create
            </button>
